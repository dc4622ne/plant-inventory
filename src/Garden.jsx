...
export default function Garden({ beds, onChange, initialFilter, onDirtyChange }) {
  ...
  const bedDirty = showBedForm && JSON.stringify(bedDraft) !== JSON.stringify(bedBaseline);
  const cropDirty = Boolean(selectedBed) && JSON.stringify(cropDraft) !== JSON.stringify(cropBaseline);
  ...
  function submitBed(event) {
    ...
    const nextBed = { ...bedDraft, id: editingBedId || makeId(), name: bedDraft.name.trim() };
    save(editingBedId ? beds.map((bed) => bed.id === editingBedId ? nextBed : bed) : [...beds, nextBed]);
    setBedDraft(emptyBed); setEditingBedId(''); setShowBedForm(false);
  }
  function editBed(bed) { if (!confirmDiscard()) return; setBedDraft({ ...emptyBed, ...bed }); setEditingBedId(bed.id); setShowBedForm(true); setSelectedId(''); }
